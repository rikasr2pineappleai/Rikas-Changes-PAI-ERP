 ...
 */
async function checkLeaveBalance(user_id, leave_type_id, requested_days, currentRequestId = null) {
  try {
    ...
    if (totalAfterRequest > totalAllocated) {
      ...
      throw new Error(`Insufficient leave balance for ${leaveTypeName}. You have ${remainingDays} days remaining, but are requesting ${requested_days} days.`);
    }
...
exports.createHoursPermissionLeave = async (req, res) => {
  ...
  try {
    ...
    if (timeDiffMinutes <= 0 || timeDiffMinutes > 180) {
      await t.rollback();
      return res.status(400).json({ error: 'Time range must be within 3 hours' });
    }
    ...
    if (reason !== undefined && reason !== null) {
      ...
      if (reason.length > 255) {
        await t.rollback();
        return res.status(400).json({ error: 'reason must be at most 255 characters' });
      }
      reasonToSave = reason;
    }
    ...
    await t.commit();
    return res.status(201).json({ message: 'Hours permission leave request created', data: created });
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    if (leave_mode === 'full_day') {
      if (numDays <= 0) {
        await t.rollback();
        return res.status(400).json({ error: 'Full day leave requests must have a positive number of days' });
      }
    }
  ...
  try {
    ...
    if (req.body.adminReason !== undefined) {
      ...
      if (typeof adminReason === 'string' && adminReason.length > 255) {
        await t.rollback();
        return res.status(400).json({ error: 'adminReason must be at most 255 characters' });
      }
    ...
    // ===== REJECT / CANCEL =====
    else if (lowerCaseNewStatus === 'rejected' || lowerCaseNewStatus === 'cancelled') {
    ...
    // ===== BACK TO PENDING =====
      ...
      await leaveReq.update(
        { status: lowerCaseNewStatus, approved_by: null, ...(reasonToSave !== undefined ? { reason: reasonToSave } : {}), ...(adminReasonToSave !== undefined ? { adminReason: adminReasonToSave } : {}) },
        { transaction: t }
      );
    }
...
exports.updateAdminReason = async (req, res) => {
  ...
  try {
    ...
    if (adminReason !== undefined && adminReason !== null && typeof adminReason !== 'string') {
      await t.rollback();
      return res.status(400).json({ error: 'adminReason must be a string or null' });
    }
    if (typeof adminReason === 'string' && adminReason.length > 255) {
      await t.rollback();
      return res.status(400).json({ error: 'adminReason must be at most 255 characters' });
    ...
    await leaveReq.update({ adminReason: adminReason === null ? null : adminReason }, { transaction: t });
...
exports.getAll = async (req, res) => {
  try {
    ...
    const result = await LeaveRequest.findAndCountAll({
      ...
      attributes: { include: ['adminReason', 'start_date', 'end_date', 'start_time', 'end_time', 'upload_document'] } // Include all necessary fields in the response
    });
...
exports.createHalfdayLeave = async (req, res) => {
  ...
  try {
    ...
    const typedLeaveTypeId = typeof leave_type_id === 'string' ? parseInt(leave_type_id, 10) : leave_type_id;
    ...
    if (typedLeaveTypeId === null || typedLeaveTypeId === undefined || isNaN(typedLeaveTypeId) || typedLeaveTypeId <= 0) {
    ...
    if (leave_mode !== 'Half Day' && leave_mode !== 'half_day') {
      await t.rollback();
      return res.status(400).json({ error: 'Invalid leave_mode for half day request' });
    }
    ...
    if (!leaveType) {
      await t.rollback();
      return res.status(400).json({ error: `Leave type with ID ${typedLeaveTypeId} not found` });
    }
    ...
    await t.commit();
    return res.status(201).json({ success: true, message: 'Half day leave request created', data: created });
...
exports.createCompulsoryLeave = async (req, res) => {
  ...
  try {
    ...
    await t.commit();
    return res.status(201).json({ success: true, message: 'Compulsory leave request created', data: created });
...
exports.getByUser = async (req, res) => {
  try {
    ...
    if (!isAdmin && currentUserId !== userId) {
      return res.status(403).json({ error: 'Access denied. You can only access your own leave requests.' });
    }
 ...
 * Get the uploaded document for a leave request
 */
exports.getLeaveDocument = async (req, res) => {
  try {
    ...
    if (!leaveReq.upload_document) {
      return res.status(404).json({ error: 'No document uploaded for this leave request' });
    }
    ...
    // Find file that matches the leave request pattern (contains leave request ID)
    const documentFile = files.find(file => 
      file.includes(`leave_${id}`) || 
      file.includes(id) ||
      file.startsWith(`${id}-`)
    );
    ...
    const filePath = path.join(uploadDir, documentFile);
    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ error: 'Document file not found' });
    }
    
    // Set appropriate headers for file download
    const fileExtension = path.extname(documentFile).toLowerCase();
    ...
    res.setHeader('Content-Disposition', `inline; filename="${documentFile}"`);
    
    const fileStream = fs.createReadStream(filePath);
    fileStream.pipe(res);
    
  } catch (err) {
    console.error('getLeaveDocument error', err);
    return res.status(500).json({ error: 'Internal server error', details: err.message });
  }
};
...
exports.getUserCalendarData = async (req, res) => {
  try {
    ...
    console.log('getUserCalendarData - userId:', userId, 'currentUserId:', currentUserId, 'req.user exists:', !!req.user);
    ...
    if (!isAdmin && currentUserId !== userId) {
      ...
      return res.status(403).json({ error: 'Access denied. You can only access your own leave calendar data.' });
    }
    ...
    for (const request of result) {
      ...
      if (isNaN(startDate.getTime())) {
        console.error(`Invalid start date for leave request ${request.id}:`, request.start_date);
        continue; // Skip this request if start date is invalid
      }
      
      if (request.end_date && isNaN(endDate.getTime())) {
        console.error(`Invalid end date for leave request ${request.id}:`, request.end_date);
      ...
      const end = request.end_date ? new Date(request.end_date) : new Date(startDate);
