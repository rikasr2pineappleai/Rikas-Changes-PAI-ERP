...
const Halfday = ({ onClose = () => {}, onRefresh = null }) => {
  ...
  function positionReasonMenu() {
    ...
    const menuWidth = Math.max(minWidth, Math.min(rect.width, viewportAvailable));
  ...
  function handleSubmit(e) {
    ...
    if (!displayDate.trim()) { setDateError('Date is required.'); hasError = true; }
    else {
      const parsed = parseDisplayToIso(displayDate.trim());
      if (!parsed) { setDateError('Invalid date format.'); hasError = true; }
      else setDateISO(parsed);
    }

    if (!startTime) { setStartTimeError('Leave session is required.'); hasError = true; }

    if (!reasonOption) { setReasonError('Reason is required.'); hasError = true; }
    else if (reasonOption === 'Others' && !reason.trim()) { setReasonError('Reason is required.'); hasError = true; }
    else setReasonError('');

    if (hasError) return;

    const finalReason = reasonOption === 'Others' ? reason.trim() : reasonOption;
    ...
    createHalfdayLeaveRequest(leaveData)
      .then(data => {
        if (data.success) {
          showToast('Half day leave request submitted successfully!', 'success');
          ...
          showToast('Error submitting request: ' + (data.error || 'Unknown error'), 'error');
        }
      })
      .catch(err => {
        ...
        <form className="hd-form" onSubmit={handleSubmit}>
          ...
          <div className="hd-fields-row">
            ...
            <div className="hd-field">
              ...
              <div className={`hd-input hd-select ${startTimeError ? 'hd-invalid' : ''}`} >
                <button
                  type="button"
                  ...
                  ref={timeToggleRef}
                >
                  <span className={startTime ? '' : 'hd-select-placeholder'}>{startTime || 'Select session'}</span>
                  <img src={hselecticon} alt="" className={`hd-select-icon ${isTimeOpen ? 'hd-rotated' : ''}`} aria-hidden="true" />
                </button>
          ...
          <div className="hd-field" style={{ marginBottom: 12 }}>
            <div className={`hd-input hd-select hd-reason-select ${reasonError ? 'hd-invalid' : ''}`}>
              ...
              >
                {reasonOption === 'Others' ? (
                  <input
                    ref={reasonInputRef}
                    className={`hd-inline-input ${reasonError ? 'hd-invalid' : ''}`}
                    value={reason}
                    onChange={(e) => setReason(e.target.value)}
                    onBlur={() => {
                      // validation on blur, same logic as before
                      if (reasonOption === 'Others') {
                        if (!reason.trim()) setReasonError('Reason is required.');
                        else setReasonError('');
                  ...
                  <span className={!reasonOption ? 'hd-select-placeholder' : ''}>
                    {reasonOption || 'Select leave reason'}
                  </span>
                )}

                <img src={hselecticon} alt="" className={`hd-select-icon ${isReasonOpen ? 'hd-rotated' : ''}`} aria-hidden="true" />
              </div>
