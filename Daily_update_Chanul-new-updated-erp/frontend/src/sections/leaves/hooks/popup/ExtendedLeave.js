...
export default function ExtendedLeave({ onClose = () => {} }) {
  ...
  function handleDateKeyDownStart(e) {
    ...
    const cur = visibleDateRefStart.current && visibleDateRefStart.current.value ? visibleDateRefStart.current.value : '';
    const selectionLength = (visibleDateRefStart.current && visibleDateRefStart.current.selectionEnd - visibleDateRefStart.current.selectionStart) || 0;
  ...
  function handleDateKeyDownEnd(e) {
    ...
    const cur = visibleDateRefEnd.current && visibleDateRefEnd.current.value ? visibleDateRefEnd.current.value : '';
    const selectionLength = (visibleDateRefEnd.current && visibleDateRefEnd.current.selectionEnd - visibleDateRefEnd.current.selectionStart) || 0;
    if (cur.length - selectionLength >= 10 && e.key.length === 1 && /[0-9/]/.test(e.key)) {
      e.preventDefault();
    }
  }
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      category,
      startDateISO: dateISOStart,
      startDateDisplay: formatIsoToDisplay(dateISOStart),
      endDateISO: dateISOEnd,
      endDateDisplay: formatIsoToDisplay(dateISOEnd),
      reason,
      fileName,
    };
  ...
  return (
    <div className="Epopup-overlay" ref={overlayRef} onMouseDown={handleOverlayClick}>
      <div className="Epopup" role="dialog" aria-modal="true" aria-labelledby="Epopup-title">
        <button className="Eclose-btn" type="button" onClick={onClose} aria-label="Close">
          <span className="Eclose-circle">
            <img src={Ecloseicon} alt="close" />
          </span>
        </button>

        <h2 className="Etitle" id="Epopup-title">New leave request</h2>
        ...
        <form className="Eform" onSubmit={handleSubmit} noValidate>
          ...
          <div className="Eselect">
            <input
              ...
              readOnly
              value={category}
              onClick={() => { /* placeholder for open dropdown if needed */}}
              aria-label="Leave category"
            />
            <div className="Eselect-icon-wrapper" onClick={() => { /* toggle if you add dropdown */ }}>
            ...
            <div>
              ...
              >
                ...
                <input
                  ...
                  onInput={(e) => { if (!composingRefStart.current) handleSanitizeAndSetStart(e.target.value); }}
              ...
              {dateErrorStart ? <div className="E-error" role="alert">{dateErrorStart}</div> : null}
            </div>
            ...
            <div>
              ...
              >
                ...
                <input
                  ...
                  onInput={(e) => { if (!composingRefEnd.current) handleSanitizeAndSetEnd(e.target.value); }}
              ...
              {dateErrorEnd ? <div className="E-error" role="alert">{dateErrorEnd}</div> : null}
            </div>
          </div>
          ...
          />
          {reasonError ? <div className="E-error" role="alert">{reasonError}</div> : null}
          ...
          <div className="Eupload" onClick={handleFileClick}>
            <span className="Eupload-fake">{fileName || 'Upload your medical certificate'}</span>
            <input
              type="file"
